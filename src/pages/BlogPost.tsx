...
const BlogPost: React.FC = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background-light dark:bg-background-dark">
      <main className="grow flex justify-center px-4 sm:px-8 md:px-16 lg:px-24 xl:px-40 py-10">
        <article className="max-w-200 w-full">
          <Link
            to="/blog"
            className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 hover:text-primary transition-colors mb-6 group"
          >
          ...
          <img
            src={article.image}
            alt={article.title}
            className="w-full h-75 md:h-100 object-cover rounded-3xl shadow-lg mb-8"
          />
                ...
                {section.list && (
                  <div className="grid grid-cols-1 gap-6 mt-4">
                    {section.list.map((item, i) => (
                      <div
                        key={i}
                        className="bg-white dark:bg-background-dark/40 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 transition-all hover:shadow-md"
                      >
