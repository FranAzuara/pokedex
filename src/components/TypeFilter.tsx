...
const TypeFilter: React.FC = () => {
  ...
  const selectedGens = searchParams.get("generations")?.split(",").filter(Boolean) || [];
    ...
    <div className="flex flex-col items-center w-full py-5">
      <div className="layout-content-container flex flex-col w-full max-w-[1200px] px-4 sm:px-8 md:px-16 lg:px-24">
        <div className="flex items-center justify-between px-4 pb-4">
          <h2 className="text-gray-900 dark:text-white text-xl font-bold">Filters</h2>
          {hasFilters && (
            <button
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="flex-1 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden h-fit">
            ...
            >
              <div className="flex items-center gap-2">
                <span className="text-gray-900 dark:text-white font-bold">Filter by Type</span>
                <span className="text-xs text-gray-500 bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded-full">
                  {selectedTypes.length}/3
                </span>
              </div>
              <span className={`material-symbols-outlined transition-transform duration-300 ${isTypesOpen ? "rotate-180" : ""}`}>
                expand_more
              </span>
            </button>
          ...
          <div className="flex-1 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden h-fit">
            ...
            >
              <div className="flex items-center gap-2">
                <span className="text-gray-900 dark:text-white font-bold">Filter by Generation</span>
                <span className="text-xs text-gray-500 bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded-full">
                  {selectedGens.length}/2
                </span>
              </div>
              <span className={`material-symbols-outlined transition-transform duration-300 ${isGensOpen ? "rotate-180" : ""}`}>
                expand_more
              </span>
            </button>
